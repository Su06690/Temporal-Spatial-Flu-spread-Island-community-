...
data1 <- outbreaker_data(dates = finalData$onset_date, #try with visit dates here since the Onset dates were not recorded for first
                         ...
                         genotype = finalData$rapid_diagnostic_test, #Genotype
...
config3a <- create_config(data = data3, 
                         find_import = TRUE,
                         n_iter = 20000, #Iteration number: main run
                         n_iter_import = 10000, #Iteration number: short run
                         sample_every = 50, 
...
import<- tm_shape(maps_combined) +
  tm_polygons(
    ...
    labels = c("0-1","1-5", "5-10", "10-20", "20-30"),
...
region_vac<- read.csv("data/vaccine coverage.csv")
reg_vac_cov<- read.csv("data/covariates.csv")


#hh<-read.csv("data/household_size.csv") 
#mean_hh<- read.csv("data/mean_hh.csv")  ## mean household per district calculated from 2010 census

test2<-cbind(n_sec_out3,dat.test)
df_out3<- as.data.frame(test2)

df<-merge(reg_vac_cov,pop, all.x = TRUE)
df_new<- merge(finalData, df,  all.x=TRUE)
df_new<- cbind(df_new, df_out3)
...
##vaccination status
df_new$vac_his [df_new$n_dose_vaccine_reported  >= 1] = 1       
df_new$vac_his [df_new$n_dose_vaccine_reported  == 0] = 0 
sum(is.na(df_new$vac_his))   ##71 missing values

df_new$vac_his <- factor(df_new$vac_his, levels=c(0,1), labels=c("No", "Yes"))
summary(df_new$vac_his)


#regional vaccination

df_new<- cov_cat[df_new$cov <20] = "<20"
df_new<- cov_cat[df_new$cov >=20 & df_new$cov <40] = "20-39"
df_new<- cov_cat[df_new$cov >=40 & df_new$cov <60 ] = "40-59"
df_new<- cov_cat[df_new$cov >=60] = ">600"

#pop_dens
df_new$pop_den_cat [df_new$pop_dens >=0 & df_new$pop_dens <150] = "<150"
df_new$pop_den_cat [df_new$pop_dens >=150 & df_new$pop_dens <350] = "150-349"
df_new$pop_den_cat [df_new$pop_dens >=350] = "350 and above"
...
reg_vac_lms <- lapply(1:ncol(n_sec_out3), function(x) glm.nb(n_sec_out3[,x] ~ log(df_new$vac_his),na.action=na.omit))
pop_den_lms<- lapply(1:ncol(n_sec_out3), function(x) glm.nb(n_sec_out3[,x] ~ df_new$pop_dens,na.action=na.omit))

lms <- lapply(1:ncol(n_sec_out3), function(x) glm.nb(n_sec_out3[,x] ~ df_new$n_age_group+ 
                                                              df_new$vac_his+ 
                                                       df_new$mean_hh+  #mean household
                                                       df_new$season+    #season
                                                       log(df_new$cov)+  # regional vaccine coverage
                                                       df_new$pop_den_cat,  # population density per region
                                                     na.action=na.omit))


summary(glm.nb(n_sec_out3[,250] ~ df_new$n_age_group+
                 df_new$vac_his+
                 df_new$mean_hh+
                 df_new$season+
                 log(df_new$cov)+
